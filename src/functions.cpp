...
const unsigned short BUFF_SIZE = 29;
...
int read_bytes(char  (&read_buf)[BUFF_SIZE],int & serial_port , int & numIterations) {
		...
		//maxfd = MAX (serial_port, 0)+1;  /* maximum bit entry (fd) to test */
		FD_SET(serial_port, &readfs);
		res = select(serial_port+1, &readfs, NULL, NULL, &Timeout);
...
void getPressuresLH(char  (&read_buf)[BUFF_SIZE], int & low_pressure, int & high_pressure){
	...
	int pressure_l = ((int)(*(unsigned char*)(&read_buf[3]))<<8) | ((int)(*(unsigned char*)(&read_buf[4])));
    int pressure_h = ((int)(*(unsigned char*)(&read_buf[5]))<<8) | ((int)(*(unsigned char*)(&read_buf[6])));
...
void editWriteBuf(char (&temp)[29], ModeHandler mh){
	temp[0] = 0x02;
	temp[1] = 0x77;
	temp[2] = 0x01;
	temp[3] = (mh.getRelayStart() ? 0x01 : 0x00);
	temp[4] = (mh.getRelayStop() ? 0x01 : 0x00);
	temp[5] = (mh.getRelayBleed() ? 0x01 : 0x00);
	temp[6] = (mh.getRelayMotor() ? 0x01 : 0x00);
	temp[7] = (mh.getRelayPump() ? 0x01 : 0x00);
	temp[8] = (mh.getRelayChiller() ? 0x01 : 0x00);
	for(int i=0; i<19;i++) {
		temp[i+9] = 0x00;
	} 
	temp[28] = 0x03;;
...
void editPollWriteBuf(char (&temp)[4]){
	temp[0] = 0x02;
	temp[1] = 0x55;
	temp[2] = 0x01;
	temp[3] = 0x03;
...
void write_bytes(int & serial_port, char (&temp)[29]){
	write(serial_port, temp, 29);//sizeof(temp));
}

int write_poll_bytes(int & serial_port, char (&temp)[4]){
	return(write(serial_port, temp, 4));//sizeof(temp));
}
...
int usb_port(int & serial_port) {
	...
	if (serial_port < 0) {
		...
		tty.c_cc[VMIN] = 29; //wait for BUFF_SIZE bytes to fill then return
...
string createJsonDataString(char  (&read_buf)[BUFF_SIZE], int pressure_low, int pressure_high, ModeHandler mh){
	int temp=0;
	...
	for(int p= 0;p < 6; p++){
		stringstream ss;
		ss.clear();
		ss << hex << setfill('0') << setw(2)  << (int)(*(unsigned char*)(&read_buf[p+7])); //offset by 7 to get to the relays readbuf[7-13]
		ss >> temp;
